...
class MessageReplyReceiver : BroadcastReceiver() {
	...
	fun onReceive(context: Context, intent: Intent) {
		if (AutoMessagingService.REPLY_ACTION.equals(intent.getAction())) {
			val conversationId = intent.getIntExtra(AutoMessagingService.CONVERSATION_ID, -1)
	...
	private fun getMessageText(intent: Intent): CharSequence? {
		...
		if (remoteInput != null) {
			return remoteInput!!.getCharSequence(AutoMessagingService.EXTRA_VOICE_REPLY)
		}
		return null
	}
	...
	companion object {
		...
		private val TAG = MessageReplyReceiver::class.java!!.getSimpleName()
	}
}
